...
{
    public class FleeingBehavior<T> : StateBasedAIBehaviorComponent<T>
    {
        MovementBasicsComponent movement;
        Timer timerForChange;

        public FleeingBehavior(T state, float minimumFleeingTime = 3f, params OutgoingState<T>[] outgoingStates) : base(state, outgoingStates)
        {
